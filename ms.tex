...
No reference pixel correction is applied, leaving offsets at the amplifier boundaries.}\label{fig:ampOffsetsOtisGrismSlope}
\end{figure}
...
The long dark frames used for detector characterization are 108 groups of 1 frame each read out if full frame mode and are 10.7 sec in duration for one long integration making up one exposure.
These long dark integrations are broken into 54 sequential pairs of images that are subtracted from each other (integration 1 minus integration 0, 3 minus 2, 5 minus 4 etc. in a zero-based counting scheme).
Figure \ref{fig:longDarkPhot} shows an example dark read pair from the integration (frame 55 minus frame 54).
In the raw data, there are 4 prominent vertical stripes due to offsets in the read-out amplifiers.
Additionally, there obvious correlations along the fast-read direction (horizontally) due to 1/f noise.
...
For a read pair subtraction, the ``CDS'' (correlated double sample) read noise is 13.3 e$^-$ for the A5 detector studied here.
The aperture contains 

In this example, reference pixel correction appears to introduce extra noise into a photometric aperture.
However, as an order of magnitude estimate, the total noise in these frames is $\sim$10 DN, compared with the 3$\times 10^{6}$ DN in our CV3 detector stability test.
Therefore, the read noise contributions are expected to be $\sim$ 3 ppm.
...
\includegraphics[width=.49\columnwidth]{long_dark_phot_red.pdf}
\caption{Time series of a 70 pixel radius photometric aperture obtained during a long dark exposure. The long dark exposure of 108 frames is used to construct 54 pairs of reads that are subtracted, in analogy with a series of integrations.
...
{\it Bottom Left:} An example read pair subtraction with an overlay of the apertures.
{\it Bottom Right:} AN example read pair subtraction after reduction steps with an overlay of the apertures.
\textcolor{red}{To Do: redo the time series without normalization since we want to show DN.}}\label{fig:longDarkPhot}
\end{figure*}
