...
Measured Read Noise (5 Component PCA on each amplifier) & 3,300 & 140 \\
...
The principal components from one group to another are very similar.
\textcolor{red}{\bf Is this just because they contain information from residuals after bias subtraction? Maybe we need a better bias subtraction?}
Indeed, an extra bias subtraction somewhat changes the character and repeatability of the first few principal components shown in Figure \ref{fig:pcaEigenvectorsExtraBias}.
...
\begin{figure*}[!hbtp]
\centering
\includegraphics[width=.4\columnwidth]{pca_dark_ind_amp_grp_54.pdf}
\includegraphics[width=.4\columnwidth]{pca_dark_ind_amp_grp_55.pdf}
...
Again, the eigenvectors are very similar other than vertical flips.
}\label{fig:pcaEigenvectors}
\end{figure*}
...
\caption{The first 5 principal component eigenvectors identified from group 54 (left) and group 55 (right) where each amplifier has been analyzed independently.
With a better bias subtraction, the eigenvectors are very similar other than vertical flips.
}\label{fig:pcaEigenvectorsExtraBias}
\end{figure*}
...
Principal component analysis detects variations from the mean and so the data is first subtracted by the mean and then the mean is added after PCA is complete.
...
We set the value of all bad pixels to 0 so they do not add any variance.
...
The resulting standard deviation of the time series is 2900 to 3870 DN.
...
In this experiment, the detectors are illuminated for 3 hours with fluence levels of about 300 DN/s for 53 seconds.
Thus, they are safely under saturation.
The charge trapping release becomes undetectable after 10 minutes and is well below 0.1 DN/s.
...
\caption{Persistence decays rapidly, indicating .}\label{fig:persistence}
\end{figure*}


\subsubsection{Pre-Ampflier Reset Test}
...
\begin{figure*}
{\gridline{\fig{rpf_vs_rpi_old_cds_no_refpix.pdf}{0.99\textwidth}{No Reference Pixels RPF}}}
{\gridline{\fig{rpf_vs_rpi_new_cds_with_refpix.pdf}{0.99\textwidth}{Reference Pixel RPF}}}
