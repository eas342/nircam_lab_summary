...
The angle between these lines are 67.60$\degree$, 44.5$\degree$ and 67.8$\degree$.
...
In this perspective, the projected bond angles form angles fo 67.8 $\degree$, 44.4$\degree$ and 67.8$\degree$, close to the observed crosshatch angles.
...
The crosshatch pattern is visible in a zoom-in of the pixel flat field (left).
The 2D power spectrum of the flat field (right) shows that the angles of the crosshatch pattern are similar to one possible projection of a tetrahedral (zincblende) lattice structure (blue, right).
...
The subpixel crosshatch pattern can potentially introduce time-variable noise as the point spread function drifts with telescope jitter.
...
We simulate the effect of this jitter on NIRCam grism time series observations with the following steps:
\begin{itemize}
	\item Fit the existing flat field to an crosshatch model that describes the subpixel behavior
	\item Simulate a PSF using \texttt{webbpsf} \citep{perrin2014webbpsf}
	\item Jitter the simulated PSF over with a 6 mas Gaussian
	\item Bin the simulated images into the native pixel size
	\item Divide by the flat field
	\item Measure the light curve with photometry or spectroscopic aperture
\end{itemize}
...
We model the two dimensional Fourier amplitude $f(x',y')$ for one angle as
\begin{equation}
f(x',y') = \exp{\left(-y'^2/\tau_\perp\right)} \left( \frac{a}{b + x'^2} + \frac{c}{1 + \exp{\left( (|x'| - d)/\tau_\parallel \right)} } \right),
\end{equation}
where $x'$ and $y'$ are rotated coordinates, $\tau_\perp$ is the perpendicular exponential constant, $a$ and $b$ are polynomial constants for the central peak of the Fourier plane, $c$ is an exponential amplitude constant and $d$ is the cutoff frequency with an exponential constant $\tau_\parallel$.
...
\end{equation}
We include three angles so that the total Fourier amplitude is
\begin{equation}
f(x,y) = \sum_{i=1}^{i=3} f_i(x'_i,y'_i)
\end{equation}
for $\theta_i = 0.9^\circ$, 68.7$^\circ$ and 113.2$^\circ$.
