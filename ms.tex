...
To create the periodogram, the average of all frames is subtracted to remove the bias and all pixels that deviate by more than 80 DN from the average are masked to eliminate bad pixels.
The periodogram shows many spikes at high frequency including one at the line reset time (the time to read out all the pixels in a row before moving to the next row).
...
It is clear from Figure \ref{fig:pixelTSeriesPSpec} that the 1/f noise and the periodic signatures significantly increase the power over uncorrelated white noise (green line).
...
The amplifier offsets have been removed, but the 1/f noise from these adjacent amplifiers shows different behavior.
...
NIRCam has a "GRISMC" grism that will disperse along the column direction but this mode would not allow stripe subarrays and would therefore saturate on bright targets with $K \lesssim 5.5$.
...
The box temperature leads the flux variations by 4.5 minutes, likely due to the thermal delay between the box and the LED mounted to the box.
