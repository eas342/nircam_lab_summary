...
An LED illumination source flood-illuminates the detectors and the arrays are read out in subarray mode to create time series.
...
This can be the result of common-mode systematic errors that divide out, so it is also important to check the time series against an independent detetor.
...
We also examine the time series across detectors from ``Run104''.
This approach will test whether there are systematics unique to each detector/readout electronics.
...
We do not approach the noise levels interesting to the exoplanet noise regime.
...
We also investigate a slightly longer time series for GL dewar.
...
\caption{Allans for background-subtracted rectangular photometry on the S02 and S03 detectors.
...
Subarray 6 had higher lamp current and thus higher counts and lower photon noise.
...
\begin{figure*}
\gridline{\fig{all_var_azlab_mult_exp_487_489_removelinear_True_489.pdf}{0.48\textwidth}{Allan variance for AZLab06 Long Baseline}
...
\caption{Allan variance for background-subtracted rectangular photometry on the S01 and S03 detectors.
The ratio of the two detectors' fluxes, after background subtraction, scales like $\sqrt{N}$ statistics down to the 60 ppm level where it begins to peel away from the $\sqrt{N}$ curve (green line).}\label{fig:GLAllanVarianceAcrossDetectorsSub6}
\end{figure*}
...
While 1/f noise substantially increases the noise over the assumption that each \textit{pixel's} read noise is statistically independent, each \textit{integration} is statistically independent.
...
The results is that the illumination pattern is not constant in time, with a subtle band centered at X$\approx$ 730 pixels that varies in amplitude.

\begin{figure*}
\gridline{\fig{azlab06_illum_baseline_plot_region.pdf}{0.49\textwidth}{Region over which the pixels are averaged and plotted}}
\gridline{\fig{azlab06_illum_baseline_slope_flat_fielded_profile_plot}{0.49\textwidth}{Flat-fielded count rates during the early and late parts of the baseline test}}
\gridline{\fig{azlab06_illum_baselin_differential_profile_plot.pdf}{0.49\textwidth}{Differential fluxes during the early and late parts of the baseline test}}
\caption{There are subtle illumination pattern changes visible when average large chunks of data during the baseline test.
There is a vertical band of illumination that changes from the early versus late parts of the time series.}\label{fig:baselineIllumChange}
\end{figure*}
