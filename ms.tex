...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=.9\columnwidth]{ideal_photodiode.pdf}
\caption{A cartoon schematic of a H2RG detector inspired by \citet{smith2008imgPersistence,tulloch2018persistenceH2RG}.
The green and tan regions depict the N-type and P-type semiconductors.
The undepleted zones of the semiconductor (bright green and dark blue) contain mobile charge carriers whereas the depletion region (solid green and tan zones) have no mobile carriers and all electrons are in the valence state.
...
Charged traps for negative and positive carriers (depicted as round circles) will capture charge before it can flow to the undepleted regions of the detector.}\label{fig:npSchematic}
\end{figure}
...
The accumulated charge from absorbed photons in the detector is measured with a multiplexor (MUX) that form a readout integrated circuit (ROI).
...
There are two possible explanations for why the WLP8SUB mode shows less scatter than the FULL modes, (1) that the reduction of read noise improves the stability of the signal and (2) that the lamp is variable on short time scales of a frame (10.74 seconds).
...
If this is uncorrelated across the 70 pixel aperture source radius, the read noise on the total aperture is $RN_{tot} = \sqrt{N_{s,px}} RN_1 = \sqrt{\pi} r_{s,px} RN_1$ = 124 * 15 e$^-$ = 1861 e$^-1 \approx 930 $DN.
...
\subsection{Lessons Learned from CV3 Tests}
...
Figure \ref{fig:otisPSFs} shows the point spread functions for the OTIS tests.

\begin{figure*}
\gridline{\fig{wlp8_otis_psf.pdf}{0.49\textwidth}{WLP8 Image}
          \fig{grism_otis_psf.pdf}{0.49\textwidth}{Grism Image}
          }
\caption{Example OTIS  images obtained simultaneously during the long duration time series.
Due to the OTIS optics set up, the short wavelength WLP8 image does not nearly represent the hexagonal PSF from CV3 or flight.}\label{fig:otisPSFs}
\end{figure*}
