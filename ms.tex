...
The surface morphology of the HgCdTe crystal shows that the crosshatch patterns are are oriented along the intersection of the (211) growth plane of the crystal and the 8 HgCdTe slip planes \citep{chang2008surfaceMorphologyHgCdTe}.
The relative angles of the HgCdTe slip planes and (211) growth plane are 44.42$\degree$, 67.79$\degree$ and 67.79$\degree$, very close to the observed crosshatch angles.
These surface topological variations with crosshatch patterns are likely related to the observed patterns in the flat fields, which are caused by quantum efficiency variations.
...
\caption{Stripe mode (4 output amplifiers) gives better precision than window mode (one amplifier).}\label{fig:stripeVsWindowAZlab04}
\end{figure*}
...
\caption{Time series for many apertures on each detector, binned into 70 equal bins.}\label{fig:manyApPerDetector}
\end{figure*}
...
\begin{figure*}
{\gridline{\fig{all_variance_removelinear_False_487.pdf}{0.45\textwidth}{Allan Variance}
\fig{all_variance_removelinear_True_487.pdf}{0.45\textwidth}{Allan Variance}}}
