...
These three angles are similar (with 0.1$\degree$) to one projection of a tetrahedral (zincblende) lattice structure of HgCdTe (bottom).
}\label{fig:crossHatchA5}
\end{figure}

The crosshatch pattern is wavelength dependent, as seen in Figure \ref{fig:crossHatchWavelengthDep}, where the 
...
\subsubsection{Subpixel Crosshatching Simulation}
...
\begin{itemize}
	\item Fit the existing flat field to an crosshatch model in the Fourier domain
...
We model the crosshatch pattern in the Fourier space because it has a smoother pattern as a function of spatial frequency as seen in Figure \ref{fig:crossHatchA5}.
We model the two dimensional Fourier amplitude $f(x_i',y_i')$ for one angle $\theta_i$ as
\begin{equation}
f(x'_i,y'_i, a_i, b,c) = a \exp{\left(- |x_i'| / b \right)} \left( \frac{1}{(0.5 c)^2 + y'^2} \right),
\end{equation}
where $x'$ and $y'$ are rotated coordinates, $a$ is the amplitude, $b$ is the parallel exponential constant and $c$ is the Lorenzian full width at half maximum.
For each of the three angles, there is a set of rotated coordinates centered at $(x_0,y_0)$= (1024,1024) following:
\begin{equation}
x'(\theta_i) = (x - x_0) \cos{\theta_i} + (y - y_0) \sin{\theta_i}
\end{equation}
and
\begin{equation}
y'(\theta_i) = -(x - x_0) \sin{\theta_i} + (y - y_0) \cos{\theta_i}.
\end{equation}
We include three angles so that the total power spectral density is
\begin{equation}
f(x,y) = \sum_{i=1}^{i=3} f_i(x'_i(\theta_i),y'_i(\theta_i),a_i,b,c).
\end{equation}
...
We only fit the region of the power spectral density above r=100 to focus on the high frequency component of the flat field where the crosshatch is most prevalent.
For the F300M filter and ALONG detector, we find 
for $\theta_1 = 0.9013 \pm 0.0004 \degree, \theta_2 = 68.6041 \pm 0.0007 \degree, \theta_3 = 113.0741 \pm 0.0007, a_1 = 6.9 \pm 0.2 \times 10^6, a_2 = 4.0 \pm 0.1 \times 10^6, a_3 = 4.4 \pm 0.1 \times 10^6, b=666 \pm 1 $ px and $c=4.9933 \pm 0.0004$ px.
The uncertainties were simply derived from the diagonals of the covariance matrix and likely are underestimated.


We take this model with best-fit parameters for the F300M filter and extend the exponential function out to an oversampling factor of 60 times the LW pixels.
...
\caption{
The crosshatch pattern changes as a function of wavelength, possibly because the shorter wavelengths resolve the structure better, and thus have Fourier amplitudes.
...
The detectors are read in full frame mode so that each frame is 10.7 sec in duration with 4 output amplifiers in use, thus the total time for the exposure was 19.3 min.
...
\includegraphics[width=.32\columnwidth]{ap_labels_darkRedRowColSub_NRCALONG-DARK-7235074213_1.pdf}
...
The number of DN collected within a 70 pixel radius aperture for WLP8 data in our CV3 detector stability test with 2 groups in RAPID (ie a correlated double sample) was $DN_{tot}=3.9\times 10^7$ DN during the integration.
The peak pixel counts in this integration were 30,000 DN or about 60\% well depth for the A1 detector.
The Poisson shot noise for photons is $N_{phot} = \sqrt{DN_{tot}} / \sqrt{G} = 4,400$DN, where $G$ is the gain ($G \approx 2.0$ for the SW detectors).
...
\textcolor{blue}{New Change to do it properly: We return to the reduced slope file that has reference pixels subtracted and the bias subtracted.
Here we subtract the median value from each row in each group.
Doing the subtractions for each group gave an RMS that was 50 ppm smaller for aperture 0, 80 ppm larger for aperture 1 and 31 ppm smaller for aperture 3.}
...
This enables subtraction of 1/f noise that has length scales shorter than the row length (ie timescales shorter than $\lesssim 5$ milliseconds, which the median subtraction cannot remove.
...
\begin{figure*}[!hbtp]
\centering
\includegraphics[width=.37\columnwidth]{ap_labels_darkRedRowColSub_NRCALONG-DARK-7235074213_1.pdf}
\includegraphics[width=.37\columnwidth]{ap_labels_darkRedsmoothedRowKernel_NRCALONG-DARK-7235074213_1.pdf}
\includegraphics[width=.37\columnwidth]{zoom_long_dark_darkRedRowColSub}
\includegraphics[width=.37\columnwidth]{zoom_long_dark_darkRedsmoothedRowKernel.pdf}
\caption{The same apertures as Figure \ref{fig:longDarkPhot}, but with subtraction by a smoothed image along the rows.
Subtraction by the smoothed image has a dramatic effect on reducing correlated 1/f read noise.
