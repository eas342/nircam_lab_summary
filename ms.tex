...
\subsubsection{Other Reference Channel Subtraction}
We note that the 4 channels have very similar 1/f correlation behavior, as shown in Figure \ref{fig:darkPixelTimeSeries}.
...
However, this increase in read noise on a per-pixel level might be acceptable if it decreases the correlated errors along the rows.

This method increases the noise over a row-by-row median subraction.
...
This means the the correlations within an amplifier are not efficiently subracted (\textbf{\textcolor{red}{or there was an error in the code. I'm surprised that it didn't make a bigger difference.}}).
\begin{figure*}[!hbtp]
\centering
\includegraphics[width=.4\columnwidth]{pixeltime_series.pdf}
\caption{The time series for all 4 amplifiers is very similar due to shared 1/f noise behavior.
This is visible in a short segment of the time series of all pixels for an example group up the ramp (53) in a dark ramp where the bias has already been subtracted.}\label{fig:darkPixelTimeSeries}
\end{figure*}
