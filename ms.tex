...
Measured Read Noise (covariance-weighted summation)	& & & 854 & 231 \\
\enddata
...
It is clear that 1/f noise causes covariance most prominently along the spectral direction (the fast-read direction), with a covariance nearlynear 36~$(e^-)^2$.
Surprisingly, there is also a covariance between spatial pixels (the slow read direction) of $\sim$20 $(e^-)^2$.

\begin{figure}[!hbtp]
\centering
\includegraphics[width=.6\columnwidth]{spec_spatial_cov.pdf}
\caption{The read noise shows significant correlations between pixels (unlike the traditional assumption that each pixel's noise is statistically independent).
...
The detector's diagonal elements are consistent with expected performance of $\approx 13^2 (e^-)^2=169 (e^-)^2$.
Spatially, the pixels' have a strong covariance of 36~$(e^-)^2$ and a somewhat smaller spatial covariance of 20$(e^-)^2$.}\label{fig:covSpecSpatial}
\end{figure}

\begin{figure}[!hbtp]
\centering
\includegraphics[width=.6\columnwidth]{cross_sections_cov.pdf}
...
This optimal weighting significantly increases the contributions from the inner pixels 

\begin{figure}[!hbtp]
\centering
\includegraphics[width=.5\columnwidth]{cov_weighted_weights.pdf}
