...
Correlated electronic noise (1/f noise) in the detectors can limit the sensitivity of grism time series for a given frame (300 ppm to 1000 ppm depending on the extraction method), but will average down like $\sqrt{N}$ for $N$ frames.
...
Future performance tests during commissioning and the early release science will still be needed to assess the noise floor on stars, which are more stable than the available LED lamps used in lab-based tests.
\end{abstract}
...
We also used dewars at the Steward Observatory at the University of Arizona used for time series stability tests.
...
CV 3       & NASA Goddard  & 2016       & 500   	& LED           & WLP8 imaging & Y & Asic \\
OTIS       & NASA Johnson  & 2017       &      	& Continuum     & Darks & Y & Asic\\
OTIS       & NASA Johnson  & 2017       &   2200    	& Continuum     & Grism spectroscopy & Y & Asic\\
OTIS       & NASA Johnson  & 2017       &   21,000    	& Continuum     & WLP8 Imaging & Y & Asic\\
GL dewar   & U of Arizona  & 2015-2016  &       	& LED	        & flood-illuminated imaging & N & Leach\\
Asic dewar & U of Arizona  & 2017-2019  &       	& LED           & flood-illuminated imaging & N & Asic\\
\enddata
...
\tablecaption{Summary of Noise Sources in OTIS Dark Test}
\tabletypesize{\footnotesize}
%\tablecolumns{7}
%\tablenum{2}
\tablewidth{0pt}
...
\begin{equation}
w_j = P_j \sum_{i=1}^{m} \left( C^{-1}\right)_{ij},\label{eq:covarianceWeighted}
\end{equation}
where $C^{-1}$ is the inverse of the covariance matrix between pixels $i$ and $j$ and $m$ is the number of pixels in the summation.
The optimally weighted flux ($f_{opt}$) then becomes
\begin{equation}
f_{opt} = \frac{\sum_{j=1}^{m} w_j S_j}{\sum_{j=1}^{m} w_j P_j},
\end{equation}
where $S_j$ is the background subtracted flux.

% the longer form I'm not using
%the weighted flux becomes,
%\begin{equation}\label{eq:covarianceWeighted}
%f_{opt} = \frac{\sum_{i,j=0}^{m} C^{-1}_{ij} S_j P_j}{\sum_{i,j=0}^{m} C^{-1}_{ij} P_j^2},
%\end{equation}
%where $C^{-1}$ is the inverse of the covariance matrix between pixels $i$ and $j$ and $m$ is the number of pixels in the summation and $S_j$ is the background subtracted flux.
...
The ideal weighting of the spectrum from Equation \ref{eq:covarianceWeighted} depends on the covariance matrix $C_{ij}$.
...
Figure \ref{fig:covSpecSpatial} shows the covariance matrix of this wavelength bin, taken from the median of 20 regions of the detector.
The diagonal elements are consistent with published NIRCam detector performance tables with $\sigma^2 \approx 13^2 (e^-)^2=169 (e^-)^2$.
It is clear that 1/f noise causes covariance most prominently along the spectral direction (the fast-read direction), with a covariance nearly uniformly at 36~$(e^-)^2$.
Surprisingly, there is also a covariance between spatial pixels (the slow read direction) of $\sim$16 $(e^-)^2$ that falls off to $\sim 10 (e^-)^2$.

\begin{figure}[!hbtp]
\centering
\includegraphics[width=.7\columnwidth]{spec_spatial_cov.pdf}
\caption{The read noise is strongly correlated in the spectral direction (diagnoal boxes) and somewhat correlated in the spatial direction (off diagonal region).}\label{fig:covSpecSpatial}
\end{figure}

\begin{figure}[!hbtp]
\centering
\includegraphics[width=.7\columnwidth]{cov_weighted_weights.pdf}
\caption{Spectral extraction weights for simple summation, conventional optimal extraction and covariance-weighting.}\label{fig:covWeightedSummation}
\end{figure}
...
The point spread functions from CV3 most closely resembled the JWST flight point spread function and WLP8 defocused image of all ground based tests discussed in this work.
...
\subsubsection{Comparison of Subarray and Full Frame Stability Tests}

CV3 included a series of exposures about 16 hours with an LED lamp in imaging mode to test the fundamental photometric performance.
...
Similarly, NIRCam time series observations can employ a weak lens to defocus a star's light.
Figure \ref{fig:WLP8PSF} show the PSF of the +8 wave defocused images during the CV3 stability tests.

We applied the ramps-to-slopes pipeline \texttt{nchdas} to do reference pixel correction, apply a bad pixel mask and calculate slopes or differences of the reads.
The reference pixel correction uses the top/bottom pixels to remove pre-amp resets and side reference pixels to reduce the 1/f noise.
The hexagonal PSF is summed with circular aperture with a radius of 70 pixels and a background annulus spanning from 75 to 100 pixels using the \texttt{photutils} package \citep{bradley2016photutilsv0p3}.
...
The LED lamp was coupled to optical fibers to illuminate two separate detectors simultaneously with the intention of having two independent measurements to compare.
Unfortunately, vibrations and jitter in the optical setup affect the relative flux in the two fibers.
The coupling mechanism into the fibers is sensitive to the lamp position, so the flux from the two detectors is highly anti-correlated, as shown in Figure \ref{fig:CV3longTserAB}.
To redice this fiber coupling effect, we average the flux from the two detectors for the rest of this discussion.
This reduces the standard deviation of the time series to 500-1500 ppm.

\begin{figure}[!hbtp]
\centering
\includegraphics[width=.39\columnwidth]{cv3_A_vs_B_all.pdf}
\caption{The light from an LED lamp is split into two optical fibers, which are highly anti-correlated likely because vibrations move the lamp relative to the fibers to couple one fiber better than the other.
Here, the time series fit to a second order polynomial in time to remove long term trends and then the aperture flux for two detectors' fluxes are plotted against each other.
For the discussion of CV3 tests forward, we average the two detectors (A1 and B4) to reduce this effect.}\label{fig:CV3longTserAB}
\end{figure}
...
The full frame modes (FULL1 through FULL6) used 2 samples of the ramp for integration times of 21.5 seconds whereas the subarrays were tuned to have a similar integration time (21.3 sec for 20 Groups of a 320$\times$320 subarray and 25.1 seconds for 6 groups of a 640$\times$640 subarray).
...
However, the groups are fit to a line for each pixel, so the useful portion of the ramp is the time between the last and first read or 20.2 seconds for the 320$\times$320 subarray versus 10.7 seconds for the FULL1 through FULL6 tests.

It is apparent from Figure \ref{fig:CV3longTser} that the WLP8SUB test with a 320$\times$320 subarray and 20 groups of the ramp has smaller scatter on short timescales than the FULL1 through FULL6 tests.
...
The large number of groups used to fit this line ensures that the read noise is reduced as compared to the FULL 2 group tests by $2/\sqrt{N_G}$, where $N_G$ is the number of groups.
...
We consider two explanations for why the WLP8SUB mode shows less scatter than the FULL modes, 
\begin{enumerate}[noitemsep]
	\item The reduction of read noise improves the stability of the signal and\label{item:readNReasonCV3subFull}
	\item That the lamp is variable on short time scales of a frame (10.74 seconds) and sampling.\label{item:lampReasonCV3subFull}
\end{enumerate}
As listed in Table \ref{tab:noiseSummary1overf}, the theoretical uncorrelated read noise for double-correlated samples (CDS) is about 33 ppm.
However, the dark frame tests in Section \ref{sec:longDarks} show that 1/f noise significantly increases this value.
When reference pixel corrections are applied to the data and a 70 pixel source radius is used, the measured CDS read noise from the dark test is 280 ppm for the long wavelength detector.
Due to the source lamp setup discussed in Figure \ref{fig:CV3longTserAB}, we average the flux from detector A1 and B4, which lowers the read noise by $\sqrt{2}$  to 200 ppm.
As a check, we perform aperture photometry on an un-illuminated regions of the time series stability tests and average the results of the two detectors.
We find a standard deviation of 1100 to 1490 e$^-$/s or 150 ppm to 200 ppm of the 7.5$\times 10^6 e^-$/s flux, likely due to slight differences in the 1/f noise within the exposures.
So, the measured standard deviation of ~1000 ppm during the full-frame tests is due largely to lamp variations and not read noise alone.


The WLP8SUB integrations had 20 groups, so the measured read noise should be 280 ppm $\times \sqrt{2/20}$= 89 ppm.
There is no un-illuminated region of the 320$\times$320 subarray that is as big as the source aperture so there is not empirical check on this 89 ppm estimate.
The difference of 83 ppm between the measured full frame read noise and the calculated subarray is not enough to explain the full difference in precision between the two tests.
Thus, we rule out \ref{item:readNReasonCV3subFull} as the main noise source.
Instead, we conclude that the lamp has significant short-timescale variability that the faster samples used on the SUB320 test can average over.
%The take-away from the comparison between subarrays is that one can reduce the read noise and short timescale variability with more samples up the ramp and a smaller subarray.

\subsubsection{Row-by-row Background Subtraction}

Even though we rule out read noise as the dominant noise source, we investigate ways to reduce the fractional contribution of read noise.
If we use a row-by-row subtraction, as discussed in Section \ref{sec:indAmpAvg}, the read noise can be reduced.
We use a procedure that reduces the 1/f electronic noise.
First, we find the robust mean flux for each row of the background annulus with radii of 95 and 195 pixels.
We then, subtract this value from the entire row of the image and perform aperture photometry on the residual image surrounding the source with a circular aperture of 70 pixels.
This procedure reduces the measured read noise on a dark (un-illuminated) region to 60 to 100 ppm.

\subsubsection{Best Pipeline Practices from CV3 Tests}\label{sec:CV3Lessons}

\begin{figure*}
\gridline{\fig{ES-WLP8SUB_pipeline_parameters.pdf}{0.39\textwidth}{SUB320 Time Series}
\fig{{ES-FULL4_pipeline_parameters.pdf}}{0.39\textwidth}{FULL4 Time Series}}
\caption{Flat fielding, linearity corrections and intrapixel capacitance corrections have a relatively small impact on the time series precisions measured during the CV3 test.
Each table lists the measured noise (in ppm) when we turn on/off IPC correction (vertical direction), flat fielding (horizontal direction) and the linearity correction (diagonal direction).
The IPC and flat fielding corrections are negligible.
Above 50\% well depth, the HgCdTe detectors become less sensitive to photons, so applying the linearity correction naturally increases the apparent noise.
}\label{fig:CV3pipeParamsWLP8}
\end{figure*}

We investigated best practices for the pipeline and analysis by running the raw data through the pipeline and sequentially adding and subtracting steps that go into producing light curves.
...
We find that the intrapixel capacitance (IPC) correction, flat fielding and linearity correction have relatively small impacts on photometric precision.
In Figure \ref{fig:CV3pipeParamsWLP8} we show the standard deviation of the flux after averaging the A1 and B4 detectors and removing a linear trend.
We quote standard deviations for two example tests: SUB320 and FULL4.
The IPC corrections matter less than 0.01 ppm, because the deconvolution kernel conserves flux.
So long as the aperture is significantly wider than the 3$\times$3 pixel kernel, the flux in an aperture should be constant.
The flat fielding correction has less than a 2 ppm impact to the standard deviation because the image motion is at the sub-pixel level.

The linearity correction does consistently increase the signal to noise by 7 to 23 ppm but this is a natural consequence of the np photodiode in our detectors.
This is because the photodiode becomes decreasingly less sensitive to photons as it the well capacity is approached.
Note that at the extreme of saturation, the photodiode no longer senses new photons so you would measure a standard deviation of 0 ppm.
Therefore, the np photodiode naturally decreases the measured standard deviation of flux that will appear larger with a non-linearity correction.

The weak lens PSF illuminates the pixels to 15,000 to 30,000 DN or 25-50\% well depth and the non-linearity for this well depth is 0.5 to 2\%.
This means that the measured standard deviation for linearized data should be about 1\% larger than for raw data, similar to the measurements in Figure \ref{fig:CV3pipeParamsWLP8}.
We therefore conclude that the non-linearity correction makes a small difference to the final derived precision.

After experimenting with these configurations, we find the following pipeline steps are important:
\begin{itemize}
	\item Fitting a line up the ramp gives superior results to subtracting the last and first pairs of read. This is  expected because more ramps reduce the 1/f and read noise, but the disparity (500 ppm versus 1200 ppm) is too large to be due to read noise alone.
	...
	\item Row-by-row subtraction (along the detector's fast-read direction) gives superior background subtraction to traditional aperture photometry (using the mean or robust average value in a background annulus) because it reduces the read noise.
...
\begin{itemize}
	...
	\item Flat-fielding ($\lesssim 2$ ppm)
\end{itemize}
...
However, there is no feasible way to place a point source lamp far enough away to simulate the imaging of astrophysical objects onto a JWST instrument, so an alternate lamp setup is used.
