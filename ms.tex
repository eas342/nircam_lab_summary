...
It should be noted, however, that there are gradients in the bias signal within each amplifier. If we simply subtract the median value of all reference pixels within an amplifier, there are still discontinuities in the spectrum.
In addition to reference pixels, background subtracted along each column with likely remove the ampilfier boundary effect.
...
\caption{With rect backub.}\label{fig:OTISRectBacksub}
\end{figure}
...
\caption{The same time series as Figure \ref{fig:GLtSeries} but with a rectangular aperture subtraction box.
