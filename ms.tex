...
The read noise for background-subtracted photometry is expected to be 1309 DN if each pixel is completely uncorrelated with the neighbors.
As an order of magnitude estimate for photometric stability, this read noise should be compared with the 3$\times 10^{6}$ DN/s (or 3$\times 10^8$ DN for a single full frame) in our CV3 detector stability test, so the read noise is expected to contribute $\sim$ 34 ppm.
...
\textcolor{red}{To do}
\begin{itemize}
	\item Make the two apertures have the same Y value
	\item Make the two apertures have the same 
	\item Put in a third aperture above the source
	\item Re-label the aperture from "src" to 0
	\item Use more pixels to reduce the 1/f noise (say median of another amplifier along the X direction)
\end{itemize}
