...
So far, we have discussed the magnitude of correlated read noise on an integration-by-integration basis, but it is also helpful to see its impact on the transit depth or eclipse depth of an exoplanet.
For context, the features size of atmospheric signatures on archetypical planets are expected to be $\sim$300 ppm for hot Jupiters and Neptunes but only 10 ppm for cool Super-Earths like K2-3 b \citep{greene2016jwst_trans}.
The 
 with clouds have molecular features of $\sim$300 ppm, most easily-observable planets, such as hot Jupiters, are expected to have molecular atmospheric features of $\sim$200 to $\sim$500 ppm \citep{schlawin2018JWSTforecasts,greene2016jwst_trans}.

\begin{figure}[!hbtp]
\centering
\includegraphics[width=.59\columnwidth]{generic_src_errors.pdf}
\caption{Bright sources, where only a handful of groups per integration are possible without saturation, can be dominated by read noise due to correlated errors and small photon noise.
