...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=.49\columnwidth]{fringing_placeholder.png}
\caption{The CV3 extracted spectrum shows dramatic structure.
Is this fringing?}\label{fig:cv3ExtractedSpectrum}
\end{figure}
...
The JWST fine steering mirror (FSM) is designed to correct any telescope pointing errors that are not fully corrected by the reaction wheels.
The JWST attitude control system team provided a simulation of the expected ``line of sight'' pointing performance shown in Figure \ref{fig:tserPointingFull}. The line of sight pointing does not include thermal expansion and contraction between instruments that occurs on $\sim$hour long timescales.
The simulation also ignored high frequency ($>$1 Hz) that would blur the PSF but not affect the integration-to-integration time series behavior.
Typical time series integrations are $\sim$ 10 to $\sim$100 seconds in duration.
The transit durations are of order $\sim$1-3 hours.
...
These moves are designed to keep the antenna pointing at ground-based stations for telemetry or data download.
Fortunately, the errors due to High Gain Antenna moves damp quickly on $\lesssim$ 1 minute e-folding timescales.
...
Therefore, the largest contributor to pointing variations will likely be thermal expansion and contraction between JWST instruments.
...
\caption{The line of sight pointing motion is expected to be extremely stable to within a standard deviation of 2.4 mas (0.04 long wavelength pixels).
...
These pixels consistently show RC behavior and are flagged as ``Do-not-use'' for the pipeline so they may be skipped in the aperture summation (ie. applied a weight of zero in a weighted sum).
...
There are dewars at the Steward Observatory at the University of Arizona used for time series stability tests.
