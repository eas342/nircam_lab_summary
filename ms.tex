...
One could subtract the mean reference pixel in each column to reduce these gradients, shown in Figure \ref{fig:ampOffsetsOtisGrismSlope}, (third fro top panel) but given that there are only 4 reference pixels at the bottom of each column, this introduces extra noise in the subtraction.
...
The diagonal elements are consistent with NIRCam detector performance with $\sigma^2 \approx 13^2 (e^-)^2=169 (e^-)^2$.
...
Unfortunately, the coupling mechanism into the fibers is sensitive to the lamp position, so the flux from the two detectors is highly anti-correlated, as shown in Figure \ref{fig:CV3longTserAB}.
To better remove this fiber coupling effect, we average the flux from the two detectors for the rest of this discussion.
