...
The undepleted zones of the semiconductor (textured regions) contain mobile charge carriers whereas the depletion region (solid green and tan zones) have no mobile carriers and all electrons are in the valence state.
The depletion region is widest after detector reset and shrinks as the detector is illuminated with photons that create electron-hole pairs.
...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=.49\columnwidth]{long_dark_phot.pdf}
\caption{Time series of a 70 pixel radius photometric aperture obtained during a long dark exposure. The long dark exposure of 108 frames is used to construct 54 pairs of reads that are subtracted, in analogy with a series of integrations. }\label{fig:longDarkPhot}
\end{figure}
...
If this is uncorrelated across the 70 pixel aperture source radius, the read noise from two groups  (correlated double sampling) on the total aperture is $RN_{tot} = \sqrt{N_{s,px}} RN_{CDS} = \sqrt{\pi} r_{s,px} RN_{CDS}$ = 124 * 15 e$^-$ = 1861 e$^-1 \approx 930 $DN.
...
However, there is no feasible way to place a point source lamp at infinity to simulate the imaging onto a JWST instrument, so an alternate lamp setup is used.
...
A small fraction of the light is emitted at wavelengths shorter than 2.5 $\mu$m, which barely leak through.
\textcolor{red}{Was this Half Pass or Pass and a Half?}
However, the weak lens and optical setup create a very unusual point spread function for the short wavelength channel, shown in Figure \ref{fig:otisPSFs}.
...
\begin{figure*}
\gridline{\fig{wlp8_otis_psf.pdf}{0.99\textwidth}{WLP8 Image} }
\gridline{\fig{grism_otis_psf.pdf}{0.99\textwidth}{Grism Image}}
\caption{Median images of the short wavelength (SW) and long wavelength (LW) that were used simultaneously in a grism stability long time series.
...
\gridline{\fig{tser_LW_OTIS.pdf}{0.99\textwidth}{Grism Time Series}}
...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=.7\columnwidth]{subarray_config_tser.pdf}
\caption{A variety of subarrays in the OTIS stability tests organized by increasing number of pixels (and decrease number of groups up the ramp).
...
At the University of Arizona, there is a laboratory setup for assessing NIRCam flight and spare detectors.
The key difference with the Asic dewar is that the detector readout electronics are completely different, using a Leach system that is operated outside of the dewar as opposed to the Asic, which is lives right next to the detectors.
...
\gridline{\fig{ap_labels_S02illum_GLrun104.pdf}{0.32\textwidth}{Apertures}
		\fig{raw_tser_phot_S02illum_GLrun104.pdf}{0.32\textwidth}{Raw time series}
		\fig{refcor_phot_S02illum_GLrun104.pdf}{0.32\textwidth}{Ratio Time Series}}
