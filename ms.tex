...
{\it Fourth Panel:} Background subtraction with a linear fit along each column eliminates offsets between amplifiers without introducing any substantial new noise source.
\textcolor{red}{Things to try: 1) check if there's any fringing in the spectrum 2) compare scatter in spectrum to photon + read noise}
...
\caption{The integrated spectrum from Figure \ref{fig:ampOffsetsOtisGrismSlope} shows high frequency noise much far in excess of the spectrum's noise.
A periodogram shows a peak near 19 days.
...
\textcolor{red}{Thing to try: show 3 panels w/ last one using row by row and column by column}
