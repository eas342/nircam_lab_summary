...
\begin{figure}[!hbtp]
\centering
\includegraphics[width=.69\columnwidth]{plot_mmm_showTemperatures_False_AB_sides.pdf}
\caption{Time Series plot through the CV3 photometric stability campaign for the A1 and B4 detectors.
The flux on the two detectors is highly anti-correlated so the flux is averaged between them for future analyses.
The anti-correlation likely arises due to the lamp source motion that preferentially couples with one fiber and then the other.
}\label{fig:CV3longTserAB}
\end{figure}
...
It is apparent from Figure \ref{fig:CV3longTser} that the WLP8SUB test with a 320$\times$320 subarray and 20 groups of the ramp has smaller scatter on short timescales than the FULL1 through FULLQ tests.
...
\end{enumerate}
As listed in Table \ref{tab:noiseSummary1overf}, the theoretical uncorrelated noise for double-correlated samples (CDS) is about 33 ppm.
...
Due to the source lamp setup, we average the flux from detector A1 and B4, which lowers the read noise by $\sqrt{2}$  to 200 ppm.
As a check, we perform aperture photometry on an un-illuminated regions of the time series stability tests.
We find a standard deviation of 172 ppm, due to slight differences in the read noise between detectors and the illumination.
So, the measured standard deviation of ~1000 ppm during the full-frame tests is due largely to lamp variations.

The WLP8SUB integrations had 20 groups, so the measured read noise should be 280 ppm $\sqrt{2/20}$ or 89 ppm.
There is no region of the 320$\times$320 subarray that is as big as the source aperture and illuminated to empirical test this estimate.
...
This is smaller than the observed difference in standard deviation (500 ppm in subarray with 20 groups and 1020-1540 ppm in full frame in 2 groups).
...
\begin{figure*}
\gridline{\fig{ES-WLP8SUB_pipeline_parameters.pdf}{0.39\textwidth}{WLP8 Time Series}
