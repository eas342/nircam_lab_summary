...
The physical size of the N-type HgCdTe semiconductor is much larger than the P-type HgCdTe because the density of carriers is much lower in the N-type.
...
The depletion region has a net electric field pointed toward the P-type side, which sets the direction electrons and holes will migrate.
...
\includegraphics[width=.32\columnwidth]{long_dark_Raw.pdf}
\includegraphics[width=.32\columnwidth]{long_dark_Reduced.pdf}
\includegraphics[width=.32\columnwidth]{long_dark_ReducedRowColSub.pdf}
...
We assume therefore the read noise per integration is:
\begin{equation}
RN_{ap,70,75,100} = \frac{21,200 DN}{ \sqrt{N_{frames}}} \approx \frac{46,000 e^-}{ \sqrt{N_{frames}}}
\end{equation},
where $N_{frames}$ is the number of frames in an integration and a gain of 2.17 $e^-$/DN is assumed for the detector.
If there are 2 frames in an integration, we recover 15,000 DN for the case of a read pair subtraction on the A3 detector.
\textcolor{red}{\bf Make sure I scaled this correctly both for the read pair subtraction and a slope fit. Also verify with a slope fit to multiple images.}
For an integration that fills to a peak of $\sim$ 60\% or 35,000 DN, the integrated DN counts would be 10$^8$ DN, having a photon uncertainty of 9,100 DN or 110 ppm.
Therefore, the correlated read noise, if left uncorrected, will have a highly significant ($>50\%$) impact on an integration of 5 or fewer frames.
