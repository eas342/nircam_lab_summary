...
\subsubsection{Lessons Learned on the Pipeline from CV3 Tests}\label{sec:CV3Lessons}
...
We investigated best practices for the pipeline and analysis by running the raw data through the pipeline and changing the steps that go into producing light curves.
...
The IPC corrections matter less than 0.01 ppm, because the deconvolution kernel conserves flux and just moves it around.
The flat fielding correction is less than 0.3 ppm because the image motion is at the sub-pixel level.
...
At the extreme of saturation, the photodiode no longer senses new photons so you would measure a standard deviation of 0 ppm.
